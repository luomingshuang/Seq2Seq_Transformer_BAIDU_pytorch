...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, is_best):
    ...
    filename = 'checkpoint_{}_words_color.tar'.format(word_length)
    torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, 'BEST_checkpoint_{}_words_color.tar'.format(word_length))
...
def parse_args():
    ...
    #parser.add_argument('--checkpoint', type=str, default='BEST_checkpoint_9_words.tar', help='checkpoint')

    parser.add_argument('--checkpoint', type=str, default='BEST_checkpoint_6_words.tar', help='checkpoint')
